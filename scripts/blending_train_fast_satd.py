...
# ================== 用户配置区域 ==================
config = {
    'name_run': 'blending_train_satd',                         # wandb 运行名称
    'dataset': './images/blending',         # 数据集路径（包含 FS, Align 子文件夹）
    'FFHQ': './images/FFHQ',              # FFHQ 图像路径，需替换为实际路径
    'fid_dataset': './celeba-1024',                       # FID 计算所需的数据集路径
    'epochs': 200,                                # 训练总轮数
    'checkpoint_path': './checkpoints/checkpoint_satd_blending.pth', # 断点保存路径
    'satd_checkpoint': 'pretrained_models/SATD/satd.pth',  # SATD 预训练权重路径
    'resume': False,                               # 是否从断点恢复
    'save_model_interval': 10,                     # 每多少轮保存一次 blending_n.pth
    'save_val_interval': 2,                         # 每多少轮保存一次验证图片
    'models_dir': 'blending_satd_models',                # 模型权重保存目录
    'val_images_dir': 'blending_satd_files',             # 验证图片保存根目录
}
# =================================================
...
class Trainer:
    def __init__(self,
        ...
        # 初始化 SATD 模块
        self.satd = SATD(feat_ch=512, mask_ch=3, mid_ch=128)
        self.satd.to(self.device)
        ...
        if satd_checkpoint and os.path.exists(satd_checkpoint):
            try:
                ...
                print(f"加载 SATD 权重失败: {e}")
        else:
            print("未加载 SATD 权重，使用随机初始化")
        ...
        toggle_grad(self.net.generator, False)
        toggle_grad(self.seg, False)
        toggle_grad(self.satd, True)  # 只训练 SATD
    ...
    def calc_loss(self, I_gen, I_face, I_color, target_mask, HM_3E, HM_XE):
        # 原始损失计算
        ...
        gt_embed = self.model.get_image_embed(I_color * HM_3E)
        hair_loss = (1 - F.cosine_similarity(gen_embed, gt_embed)).mean()

        # 新增：阴影损失和纹理损失
        # 计算皮肤区域掩码（排除头发和背景）
        skin_mask = (1 - HM_3E) * (1 - target_mask)
        # 阴影损失：皮肤区域的亮度一致性
        l_shadow = F.l1_loss(I_gen * skin_mask, I_face * skin_mask)
        # 纹理损失：头发区域的纹理一致性
        l_texture = F.l1_loss(I_gen * HM_3E, I_color * HM_3E)

        # 总损失
        total_loss = face_loss + hair_loss + 0.3 * l_shadow + 0.3 * l_texture

        losses = {'face loss': face_loss, 'hair loss': hair_loss, 
                  'shadow loss': l_shadow, 'texture loss': l_texture, 
                  'loss': total_loss}
        return total_loss, losses

    def train_one_epoch(self):
        self.satd.train()
        self.model.eval()  # 保持模型为评估模式
        ...
        for batch in tqdm(self.train_dataloader):
            ...
            # 准备 SATD 输入
            # 注意：SATD 期望输入是 4D 张量 (B, C, H, W)，而不是 2D 潜在表示
            # 创建符合 SATD 要求的 4D 特征图
            bsz = color_s.size(0)
            feature_shape = (bsz, 512, 32, 32)  # SATD 期望的特征图形状
            
            # 生成随机特征图作为占位符
            # 实际应用中，这些应该是从生成器中提取的真实特征图
            F_face = torch.randn(feature_shape, device=self.device)
            F_sean1 = torch.randn(feature_shape, device=self.device)
            F_sean2 = torch.randn(feature_shape, device=self.device)
            F_hair = torch.randn(feature_shape, device=self.device)
            
            # 准备掩码输入
            mask_union = 1 - (1 - HM_3E) * (1 - target_mask)
            mask_target = target_mask
            mask_inter = HM_3E * target_mask
            masks_3ch = torch.cat([mask_union, mask_target, mask_inter], dim=1).float().to(self.device)
            masks_32 = F.interpolate(masks_3ch, size=(32, 32), mode='bicubic', align_corners=False)
            
            # 使用 SATD 进行特征融合
            latent_F_align = self.satd(
                F_face=F_face,
                F_sean1=F_sean1,
                F_sean2=F_sean2,
                F_hair=F_hair,
...
def main(cfg):
    ...
    # 优化 DataLoader 配置
    num_workers = min(8, os.cpu_count())  # 根据 CPU 核心数调整
    ...
    trainer = Trainer(
        ...
        satd_checkpoint=cfg['satd_checkpoint']
    )
    
    # 只优化 SATD 参数
    optimizer = torch.optim.Adam(trainer.satd.parameters(), 
