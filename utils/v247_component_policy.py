...
def _is_useful(decision: str) -> bool:
    return decision.endswith("_USEFUL")
...
def build_component_policy(overall: dict[str, object], groups: dict[str, dict[str, object]]) -> dict[str, object]:
    ...
    for component, summary_key in (("L", "l_module"), ("AB", "ab_module"), ("Shading", "shading_module"), ("Plausibility", "plausibility_module")):
        ...
        useful_count = sum(_is_useful(value) for value in group_decisions.values())
        overall_useful = _is_useful(overall_decision)
        global_enabled = overall_useful and useful_count >= 2
        disabled_groups = [name for name, value in group_decisions.items() if not _is_useful(value)]
