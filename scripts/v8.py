...
USER_LOG_IMAGE_COUNT = 40

# Keep the SATD-cleaned alignment from being darkened or distorted by color blending.
USER_LAMBDA_CLIP_HAIR_V8 = 0.0
USER_LAMBDA_COLOR_CHROMA_V8 = 8.00
USER_LAMBDA_COLOR_RGB_STATS_V8 = 4.00
USER_LAMBDA_HAIR_LUMA_KEEP_V8 = 0.80
USER_LAMBDA_TONE_RGB_KEEP_V8 = 2.50
USER_LAMBDA_TONE_LUMA_KEEP_V8 = 2.00
USER_LAMBDA_TONE_CHROMA_KEEP_V8 = 1.50
USER_LAMBDA_SATD_PRESERVE_V8 = 1.50
USER_LAMBDA_SATD_NON_DARK_V8 = 2.00
USER_LAMBDA_REMOVE_PRESERVE_V8 = 4.00
USER_LAMBDA_REMOVE_NON_DARK_V8 = 3.00
# ============================================================================
...
def masked_chroma_stats_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    pred_mean, pred_std = masked_mean_std(chroma_uv(pred), mask)
    target_mean, target_std = masked_mean_std(chroma_uv(target), mask)
    return F.l1_loss(pred_mean, target_mean) + F.l1_loss(pred_std, target_std)


def masked_rgb_stats_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    pred_mean, pred_std = masked_mean_std(rgb01(pred), mask)
    target_mean, target_std = masked_mean_std(rgb01(target), mask)
    return F.l1_loss(pred_mean, target_mean) + F.l1_loss(pred_std, target_std)
...
def prepare_item(exp, dataset_dir: Path, face_root: Path, color_root: Path):
    ...
    try:
        ...
        # Match the author's blending training setup: color supervision is trained
        # against the face->color aligned feature, so the color loss does not fight
        # a different target hairstyle.
...
class BlendingTrainerV8:
    ...
    def prepare_batch(self, batch):
        ...
        hm_xd, hm_xe = self.helper.generate_mask(i_x)
        target_mask = (1 - hm_1d) * (1 - hm_3d) * (1 - hm_xd)
        satd_protect_mask = (remove_mask + 0.35 * target_mask + 0.25 * (1 - hm_xd) * (1 - hm_3e)).clamp(0, 1)
        color_transfer_mask = (hm_xe * (1.0 - remove_mask)).clamp(0, 1)
        ...
        valid = hm_3e.flatten(1).any(dim=1) & color_transfer_mask.flatten(1).any(dim=1)
        if not valid.any():
            return None
    ...
    def calc_loss(
        ...
        hair_luma_keep_loss = masked_l1(gray(i_gen), gray(i_satd), mask_color_transfer)
        tone_rgb_keep_loss = masked_l1(i_gen, i_satd, mask_face)
        tone_luma_keep_loss = masked_l1(gray(i_gen), gray(i_satd), mask_face)
        tone_chroma_keep_loss = masked_l1(chroma_uv(i_gen), chroma_uv(i_satd), mask_face)
        satd_preserve_loss = masked_l1(i_gen, i_satd, mask_satd)
        satd_non_dark_loss = masked_non_darker(gray(i_gen), gray(i_satd), (mask_satd + mask_color_transfer).clamp(0, 1))
        remove_preserve_loss = masked_l1(i_gen, i_satd, mask_remove)
        remove_non_dark_loss = masked_non_darker(gray(i_gen), gray(i_satd), mask_remove)
        total_loss = (
            face_loss
            + USER_LAMBDA_CLIP_HAIR_V8 * clip_hair_loss
            + USER_LAMBDA_COLOR_CHROMA_V8 * color_chroma_loss
            + USER_LAMBDA_COLOR_RGB_STATS_V8 * color_rgb_stats_loss
            + USER_LAMBDA_HAIR_LUMA_KEEP_V8 * hair_luma_keep_loss
            + USER_LAMBDA_TONE_RGB_KEEP_V8 * tone_rgb_keep_loss
            + USER_LAMBDA_TONE_LUMA_KEEP_V8 * tone_luma_keep_loss
            + USER_LAMBDA_TONE_CHROMA_KEEP_V8 * tone_chroma_keep_loss
            + USER_LAMBDA_SATD_PRESERVE_V8 * satd_preserve_loss
            + USER_LAMBDA_SATD_NON_DARK_V8 * satd_non_dark_loss
            + USER_LAMBDA_REMOVE_PRESERVE_V8 * remove_preserve_loss
            + USER_LAMBDA_REMOVE_NON_DARK_V8 * remove_non_dark_loss
        )
        return total_loss, {
            "face_loss": face_loss,
            ...
            "satd_non_dark_loss": satd_non_dark_loss,
    ...
    def validate(self, epoch: int):
        ...
        print(
            ...
            f"val_chroma={avg_losses['color_chroma_loss']:.6f} "
            f"val_rgb_color={avg_losses['color_rgb_stats_loss']:.6f} "
            f"val_hair_luma={avg_losses['hair_luma_keep_loss']:.6f} "
            f"val_tone_rgb={avg_losses['tone_rgb_keep_loss']:.6f} "
            f"val_tone_luma={avg_losses['tone_luma_keep_loss']:.6f} "
            f"val_tone_chroma={avg_losses['tone_chroma_keep_loss']:.6f} "
            f"val_clip_hair={avg_losses['clip_hair_loss']:.6f} "
            f"val_satd_preserve={avg_losses['satd_preserve_loss']:.6f} "
            f"val_satd_non_dark={avg_losses['satd_non_dark_loss']:.6f} "
            f"val_remove={avg_losses['remove_preserve_loss']:.6f} "
            f"val_remove_non_dark={avg_losses['remove_non_dark_loss']:.6f}"
        )
...
def main():
    ...
    print(
        f"[blending_v8] train_on_shape_align=True use_satd_v8={USER_USE_SATD_V8} "
