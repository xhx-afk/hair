...
def main() -> None:
    ...
    selected_summary = {"count": len(selected_valid), "valid_fraction": len(selected_valid) / max(len(records), 1), "selected_components": selected_components_union, "selected_l_q50_error": selected_medians.get("l_q50_error", float("nan")), "selected_median_ab_error": selected_medians.get("median_ab_error", float("nan")), "selected_chroma_error": selected_medians.get("chroma_error", float("nan")), "selected_hue_error": selected_medians.get("stable_hue_error_deg", float("nan")), "carrier_mid_structure_corr": selected_medians.get("carrier_mid_structure_corr", float("nan")), "carrier_gradient_structure_corr": selected_medians.get("carrier_gradient_structure_corr", float("nan")), "carrier_relative_mid_energy": selected_medians.get("carrier_relative_mid_energy", float("nan")), "carrier_relative_hf_energy": selected_medians.get("carrier_relative_hf_energy", float("nan")), "strict_non_hair_max_rgb_change": selected_medians.get("strict_non_hair_max_rgb_change", float("nan")), "selected_carrier_mid_structure_corr": selected_medians.get("carrier_mid_structure_corr", float("nan")), "selected_gradient_structure_corr": selected_medians.get("carrier_gradient_structure_corr", float("nan")), "selected_strict_non_hair_max_rgb_change": selected_medians.get("strict_non_hair_max_rgb_change", float("nan")), "selected_gamut_fraction": selected_medians.get("target_hair_gamut_heavy_fraction", float("nan")), "medians": selected_medians, "carrier_contract": "PASS" if selected_contract else "FAIL"}
    (metrics_dir / "selected_summary.json").write_text(json.dumps(selected_summary, indent=2, allow_nan=True), encoding="utf-8")
    selected_global_components = list(selected_summary["selected_components"])
    policy_global_components = sorted(component for component, entry in policy.items() if entry["global_enabled"])
    if selected_global_components != policy_global_components:
        raise RuntimeError("Selected summary components diverge from component policy")
    group_selection = {group: selected_components_for_group(policy, group) for group in ("low_chroma", "normal_chroma", "high_chroma")}
    selected_policy_payload = {"selected_global_components": selected_global_components, "component_policy": policy, "group_selection": group_selection, "samples": selected_manifest}
    (metrics_dir / "selected_policy.json").write_text(json.dumps(selected_policy_payload, indent=2), encoding="utf-8")
    (args.output_root / "selected_policy.json").write_text(json.dumps(selected_policy_payload, indent=2), encoding="utf-8"); (args.output_root / "per_sample.jsonl").write_text("\n".join(json.dumps(row, ensure_ascii=True, allow_nan=True) for row in records) + "\n", encoding="utf-8")
    ...
    # Acceptance is a report of the rerun candidate, so its contract and
    # metrics must be copied from the single selected-summary source of truth.
    acceptance = {"version": "v2.47", "diagnostic_only": True, "component_attribution": {"L": summary.get("l_module", {}).get("decision"), "AB": summary.get("ab_module", {}).get("decision"), "Shading": summary.get("shading_module", {}).get("decision"), "Plausibility": summary.get("plausibility_module", {}).get("decision")}, "component_policy": policy, "selected_global_components": selected_summary["selected_components"], "selected_components": selected_summary["selected_components"], "carrier_contract": selected_summary["carrier_contract"], "selected_metrics": selected_summary["medians"], "selected_candidate_metrics": selected_summary, "selected_candidate": {"carrier_contract": selected_summary["carrier_contract"], "reference_fidelity": "PASS" if reference_gain else ("NO_GAIN" if selected_valid else "FAIL"), "hair_only_contract": "PASS" if selected_median("strict_non_hair_max_rgb_change") <= 1e-5 else "FAIL", "gamut": "PASS" if selected_valid else "FAIL"}, "overall_decision": final_decision, "selected_summary": selected_summary}
