...
USER_OUTPUT_DIR_FFHQ = Path("output/satd_train_v8_3000")

USER_SMALL_DATASET_DIR = Path("images/satd_dataset_v8_small5.6")
USER_SMALL_FACE_ROOT = Path("images/FFHQ_long")
USER_SMALL_SHAPE_ROOT = Path("images/FFHQ_short")
USER_SMALL_OUTPUT_DIR = Path("output/satd_train_v8_small5.6")
...
USER_WANDB_RUN_NAME = "satd_train_v8"
USER_WANDB_PROJECT = "HairFast-SATD-v8"
...
class SATDPairDataset_v8(Dataset):
...
class TrainerSATD_v8:
    ...
    def save_checkpoint(self, epoch: int, is_best: bool = False):
        ...
        torch.save({"satd_v8_state_dict": self.satd.state_dict()}, self.output_ckpt_dir / "satd_for_infer_v8.pth")
    ...
    def run_epoch(self, loader, training: bool, epoch: int):
        ...
        with context:
            for step, batch in enumerate(tqdm(loader)):
                steps_in_epoch += 1
                pred_images, author_images, latent_list, info_list = self.forward_batch(
                    batch,
...
def main():
    ...
    train_dataset = SATDPairDataset_v8(train_pairs, ACTIVE_FACE_IMAGE_ROOT, ACTIVE_SHAPE_IMAGE_ROOT)
    val_dataset = SATDPairDataset_v8(val_pairs, ACTIVE_FACE_IMAGE_ROOT, ACTIVE_SHAPE_IMAGE_ROOT)
    ...
    trainer = TrainerSATD_v8(train_loader, val_loader)
    trainer.train_loop()
