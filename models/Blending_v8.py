...
class Blending_v8(Blending):
    ...
    @staticmethod
    def build_v236_runtime_inputs(**kwargs):
        return build_v836_runtime_inputs(**kwargs)

    @staticmethod
    def build_v237_runtime_inputs(**kwargs):
        return build_v837_runtime_inputs(**kwargs)

    @staticmethod
    @torch.inference_mode()
    def run_v236_appearance_debug(transfer, **runtime_inputs):
        return transfer(return_aux=True, **runtime_inputs)

    @staticmethod
    @torch.inference_mode()
    def run_v237_appearance_debug(transfer, **runtime_inputs):
        return transfer(return_aux=True, **runtime_inputs)

    @staticmethod
    def build_v238_runtime_inputs(**kwargs):
        return build_v838_runtime_inputs(**kwargs)

    @staticmethod
    @torch.inference_mode()
    def run_v238_appearance_debug(transfer, **runtime_inputs):
        return transfer(return_aux=True, **runtime_inputs)

    @staticmethod
    def build_v239_runtime_inputs(**kwargs):
        return build_v839_runtime_inputs(**kwargs)

    @staticmethod
    @torch.inference_mode()
    def run_v239_appearance_debug(transfer, **runtime_inputs):
        return transfer(return_aux=True, **runtime_inputs)
    ...
    @staticmethod
    def build_v241_runtime_inputs(**kwargs):
        return build_v841_runtime_inputs(**kwargs)
    ...
    def __init__(self, opts, net=None):
        ...
        self.v241 = self.v226 and bool(getattr(self.opts, "v241_enabled", False))
        ...
        self.v237 = self.v226 and bool(getattr(self.opts, "v237_enabled", True)) and not self.v238 and not self.v239 and not self.v240 and not self.v241
        self.v236 = self.v226 and bool(getattr(self.opts, "v236_enabled", True)) and not self.v237 and not self.v238 and not self.v239 and not self.v240 and not self.v241
        self.v235 = self.v226 and bool(getattr(self.opts, "v235_enabled", True)) and not self.v236 and not self.v237 and not self.v238 and not self.v239 and not self.v240 and not self.v241
        self.v234 = self.v226 and bool(getattr(self.opts, "v234_enabled", True)) and not self.v235 and not self.v236 and not self.v237 and not self.v238 and not self.v239 and not self.v240 and not self.v241
        # V2.34 is a direct carrier path. It must not initialize or call the
        # V2.31-V2.33 matting/recomposition stack when enabled.
        self.v231 = self.v230 and bool(getattr(self.opts, "v231_enabled", True)) and not self.v234 and not self.v235 and not self.v236 and not self.v237 and not self.v238 and not self.v239 and not self.v240 and not self.v241
        ...
        if self.v241:
            transfer_cls = HairLocalRecompositionV842 if self.v2412 else HairLocalRecompositionV841
            self.v241_transfer = transfer_cls(
                ...
                illumination_radius=getattr(self.opts, "v241_illumination_radius", 11),
                carrier_mid_radius=getattr(self.opts, "v241_carrier_mid_radius", 3),
                mid_gain=getattr(self.opts, "v241_mid_gain", 1.0),
                high_gain=getattr(self.opts, "v241_high_gain", 0.95),
                chroma_mid_gain=getattr(self.opts, "v241_chroma_mid_gain", 0.35),
                ...
                version="v2.41.2" if self.v2412 else "v2.41.1",
            )
            transfer_config = self.v241_transfer.config_dict()
            print(
                f"[Blending_v8] {transfer_config.get('version', 'v2.41')} "
                f"stable-hue/gamut/face-guard enabled; config={transfer_config}"
            )
        if self.v240:
        ...
        if self.v239:
            self.v239_transfer = HairLocalRecompositionV839(
                ...
                palette_min_support=getattr(self.opts, "v239_palette_min_support", 16),
                illumination_radius=getattr(self.opts, "v239_illumination_radius", 11),
                illumination_scale=getattr(self.opts, "v239_illumination_scale", 0.9),
                anchor_hf_gain=getattr(self.opts, "v239_anchor_hf_gain", 0.9),
                contact_radius=getattr(self.opts, "v239_contact_radius", 5),
                risk_strength=getattr(self.opts, "v239_risk_strength", 0.65),
                matte_ring_radius=getattr(self.opts, "v239_matte_ring_radius", 5),
                face_guard=getattr(self.opts, "v239_face_guard", 0.35),
            )
            print(f"[Blending_v8] V2.39 intrinsic-tone calibration enabled; config={self.v239_transfer.config_dict()}")
        if self.v238:
            self.v238_transfer = HairLocalRecompositionV838(
                palette_mad_scale=getattr(self.opts, "v238_palette_mad_scale", 3.5),
                palette_min_support=getattr(self.opts, "v238_palette_min_support", 16),
                illumination_radius=getattr(self.opts, "v238_illumination_radius", 11),
                anchor_hf_gain=getattr(self.opts, "v238_anchor_hf_gain", 0.9),
                contact_radius=getattr(self.opts, "v238_contact_radius", 5),
                risk_strength=getattr(self.opts, "v238_risk_strength", 0.65),
                matte_ring_radius=getattr(self.opts, "v238_matte_ring_radius", 5),
                face_guard=getattr(self.opts, "v238_face_guard", 0.35),
            )
            print(f"[Blending_v8] V2.38 occlusion-aware matte enabled; config={self.v238_transfer.config_dict()}")
        if self.v237:
            self.v237_transfer = HairLocalAppearanceRecompositionV837(
                palette_mad_scale=getattr(self.opts, "v237_palette_mad_scale", 3.5),
                palette_min_support=getattr(self.opts, "v237_palette_min_support", 16),
                illumination_radius=getattr(self.opts, "v237_illumination_radius", 11),
                anchor_hf_gain=getattr(self.opts, "v237_anchor_hf_gain", 0.9),
                face_guard=getattr(self.opts, "v237_face_guard", 1.0),
            )
            print(
                "[Blending_v8] V2.37 hair-local appearance recomposition enabled; "
                f"config={self.v237_transfer.config_dict()}"
            )
        if self.v236:
            self.v236_transfer = HairAppearanceDecompositionTransferV836(
                chroma_radius=getattr(self.opts, "v236_chroma_radius", 9),
                chroma_residual_radius=getattr(self.opts, "v236_chroma_residual_radius", 3),
                illumination_radius=getattr(self.opts, "v236_illumination_radius", 11),
                chroma_scale=getattr(self.opts, "v236_chroma_scale", 18.0),
                chroma_gain=getattr(self.opts, "v236_chroma_gain", 1.0),
                illumination_gain=getattr(self.opts, "v236_illumination_gain", 0.35),
                max_illumination_shift=getattr(self.opts, "v236_max_illumination_shift", 20.0),
            ).to(self.opts.device).eval()
            print(
                "[Blending_v8] V2.36 hair appearance decomposition enabled; "
                f"config={self.v236_transfer.config_dict()}"
            )
        if self.v235:
    ...
    _blend_v236_carrier = _blend_v235_carrier
    ...
    def blend_images(self, align_shape, align_color, name_to_embed, **kwargs):
        ...
        if needs_blend:
            ...
            if self.v241 or self.v240 or self.v239 or self.v238 or self.v237 or self.v236 or self.v235:
            ...
            if self.v241:
                ...
                I_anchor, I_blend_256, blending_aux = self._blend_v238_carrier(
                    base_rgb=I_base_256, latent_face=latent_S_1, latent_color=latent_S_3,
                    latent_f_align=latent_F_align, target_hair_mask=HM_X,
                    target_hair_eroded=HM_XE, color_bundle=bundle,
                )
                ...
                I_anchor, I_blend_256, blending_aux = self._blend_v237_carrier(
                    base_rgb=I_base_256, latent_face=latent_S_1, latent_color=latent_S_3,
                    latent_f_align=latent_F_align, target_hair_mask=HM_X,
                    target_hair_eroded=HM_XE, color_bundle=bundle,
                )
                ...
                I_anchor, I_blend_256, blending_aux = self._blend_v236_carrier(
                    base_rgb=I_base_256, latent_face=latent_S_1, latent_color=latent_S_3,
                    latent_f_align=latent_F_align, target_hair_mask=HM_X,
                    target_hair_eroded=HM_XE, color_bundle=bundle,
                )
        ...
        if (self.v241 or self.v240 or self.v239 or self.v238 or self.v237 or self.v236 or self.v235 or self.v234) and needs_blend:
        ...
        if self.v241 and needs_blend:
            ...
            v241_runtime = build_v841_runtime_inputs(
                ...
                source_hair_mask=F.interpolate(parser_regions_v230["source_hair_mask"].float(), size=anchor_rgb01.shape[-2:], mode="bilinear", align_corners=False),
            )
            final_rgb01, v241_aux = self.v241_transfer(return_aux=True, **v241_runtime)
            I_final = final_rgb01 * 2.0 - 1.0
            blending_aux.update({"v241_runtime": v241_runtime, "v241_final_rgb": final_rgb01, **v241_aux})
        elif self.v240 and needs_blend:
            base_rgb01 = ((I_base_256 + 1.0) / 2.0).clamp(0, 1)
            anchor_rgb01 = ((I_anchor + 1.0) / 2.0).clamp(0, 1)
            reference_rgb01 = ((I_3 + 1.0) / 2.0).clamp(0, 1)
            reference_hair_hr = F.interpolate(hair_color_mask.float(), size=anchor_rgb01.shape[-2:], mode="nearest")
            ...
            face_hr = F.interpolate(parser_regions_v230["source_face_mask"].float(), size=anchor_rgb01.shape[-2:], mode="bilinear", align_corners=False)
            skin_hr = F.interpolate(parser_regions_v230["source_skin_mask"].float(), size=anchor_rgb01.shape[-2:], mode="bilinear", align_corners=False)
            v239_runtime = build_v839_runtime_inputs(
                base_rgb=base_rgb01, strong_anchor_rgb=anchor_rgb01,
                color_reference_rgb=reference_rgb01, target_illumination_rgb=base_rgb01,
                coarse_target_hair_mask=target_hair_hr, source_face_mask=face_hr,
                source_skin_mask=skin_hr, reference_hair_mask=reference_hair_hr,
            )
            final_rgb01, v239_aux = self.v239_transfer(return_aux=True, **v239_runtime)
            I_final = final_rgb01 * 2.0 - 1.0
            blending_aux.update({"v239_runtime": v239_runtime, "v239_final_rgb": final_rgb01, **v239_aux})
        elif self.v238 and needs_blend:
            base_rgb01 = ((I_base_256 + 1.0) / 2.0).clamp(0, 1)
            anchor_rgb01 = ((I_anchor + 1.0) / 2.0).clamp(0, 1)
            reference_rgb01 = ((I_3 + 1.0) / 2.0).clamp(0, 1)
            reference_hair_hr = F.interpolate(hair_color_mask.float(), size=anchor_rgb01.shape[-2:], mode="nearest")
            ...
            face_hr = F.interpolate(parser_regions_v230["source_face_mask"].float(), size=anchor_rgb01.shape[-2:], mode="bilinear", align_corners=False)
            skin_hr = F.interpolate(parser_regions_v230["source_skin_mask"].float(), size=anchor_rgb01.shape[-2:], mode="bilinear", align_corners=False)
            v238_runtime = build_v838_runtime_inputs(
                base_rgb=base_rgb01, strong_anchor_rgb=anchor_rgb01,
                color_reference_rgb=reference_rgb01, target_illumination_rgb=base_rgb01,
                coarse_target_hair_mask=target_hair_hr,
                source_face_mask=face_hr, source_skin_mask=skin_hr,
                reference_hair_mask=reference_hair_hr,
            )
            final_rgb01, v238_aux = self.v238_transfer(return_aux=True, **v238_runtime)
            I_final = final_rgb01 * 2.0 - 1.0
            blending_aux.update({"v238_runtime": v238_runtime, "v238_final_rgb": final_rgb01, **v238_aux})
        elif self.v237 and needs_blend:
            base_rgb01 = ((I_base_256 + 1.0) / 2.0).clamp(0, 1)
            anchor_rgb01 = ((I_anchor + 1.0) / 2.0).clamp(0, 1)
            reference_rgb01 = ((I_3 + 1.0) / 2.0).clamp(0, 1)
            reference_hair_hr = F.interpolate(hair_color_mask.float(), size=anchor_rgb01.shape[-2:], mode="nearest")
            ...
            v237_runtime = build_v837_runtime_inputs(
                base_rgb=base_rgb01, strong_anchor_rgb=anchor_rgb01,
                color_reference_rgb=reference_rgb01, target_illumination_rgb=base_rgb01,
                target_hair_mask=target_hair_hr, face_mask=face_hr,
                reference_hair_mask=reference_hair_hr, target_hair_alpha=target_hair_hr,
            )
            final_rgb01, v237_aux = self.v237_transfer(return_aux=True, **v237_runtime)
            I_final = final_rgb01 * 2.0 - 1.0
            blending_aux.update({"v237_runtime": v237_runtime, "v237_final_rgb": final_rgb01, **v237_aux})
        elif self.v236 and needs_blend:
            anchor_rgb01 = ((I_anchor + 1.0) / 2.0).clamp(0, 1)
            reference_rgb01 = ((I_3 + 1.0) / 2.0).clamp(0, 1)
            reference_hair_hr = F.interpolate(
                hair_color_mask.float(), size=anchor_rgb01.shape[-2:], mode="nearest"
            )
            reference_rgb01 = reference_rgb01 * reference_hair_hr + 0.5 * (1.0 - reference_hair_hr)
            target_hair_hr = F.interpolate(HM_X.float(), size=anchor_rgb01.shape[-2:], mode="nearest")
            face_hr = F.interpolate(
                parser_regions_v230["source_face_mask"].float(),
                size=anchor_rgb01.shape[-2:], mode="nearest",
            )
            illumination_rgb01 = F.interpolate(
                ((I_base_256 + 1.0) / 2.0).clamp(0, 1),
                size=anchor_rgb01.shape[-2:], mode="bilinear", align_corners=False,
            )
            v236_runtime = build_v836_runtime_inputs(
                strong_anchor_rgb=anchor_rgb01,
                color_reference_rgb=reference_rgb01,
                illumination_rgb=illumination_rgb01,
                target_hair_mask=target_hair_hr,
                anchor_hair_mask=target_hair_hr,
                face_mask=face_hr,
                reference_hair_mask=reference_hair_hr,
                illumination_hair_mask=target_hair_hr,
            )
            final_rgb01, v236_aux = self.v236_transfer(return_aux=True, **v236_runtime)
            I_final = final_rgb01 * 2.0 - 1.0
            blending_aux.update({
                "v236_runtime": v236_runtime,
                "v236_final_rgb": final_rgb01,
                **v236_aux,
            })
        elif self.v235 and needs_blend:
            anchor_rgb01 = ((I_anchor + 1.0) / 2.0).clamp(0, 1)
            reference_rgb01 = ((I_3 + 1.0) / 2.0).clamp(0, 1)
        ...
        if self.opts.save_all:
            ...
            if bundle is not None:
                ...
                if self.selective_runtime:
                    ...
                    if (self.v223 or self.v224 or self.v225 or self.v226) and blending_aux is not None and not (self.v234 or self.v235 or self.v236 or self.v237 or self.v238 or self.v239 or self.v240 or self.v241):
                    ...
                    if (self.v224 or self.v225 or self.v226) and not (self.v234 or self.v235 or self.v236 or self.v237 or self.v238 or self.v239 or self.v240 or self.v241):
                            save_gen_image(
                    ...
                    if self.v240:
                        ...
                        for name, key in (("intrinsic_tone_preview.png", "intrinsic_tone_preview_rgb"), ("final_l_low.png", "final_l_low"), ("gamut_chroma_scale.png", "gamut_chroma_scale"), ("hair_alpha_final.png", "target_hair_alpha_final"), ("final_leakage_map.png", "non_hair_leakage_map")):
                            ...
                            if value.size(1) == 1:
                                value = value.repeat(1, 3, 1, 1)
                            save_gen_image(output_dir, "Blending_v8", name, value / 100.0 * 2.0 - 1.0 if key == "final_l_low" else value * 2.0 - 1.0)
                    elif self.v238:
                        for name, key in (("allowed_hair_mask.png", "occlusion_allowed_hair"), ("hair_alpha_final.png", "target_hair_alpha_final"), ("final_leakage_map.png", "non_hair_leakage_map")):
                            value = blending_aux[key]
                            if value.size(1) == 1:
                                value = value.repeat(1, 3, 1, 1)
                            save_gen_image(output_dir, "Blending_v8", name, value * 2.0 - 1.0)
                    elif self.v237:
                        for name, key in (
                            ("hair_chroma_map.png", "target_ab"),
                            ("hair_illumination_map.png", "target_l_final"),
                            ("illumination_residual.png", "anchor_high_frequency_l"),
                            ("chroma_confidence.png", "chroma_strength"),
                            ("final_leakage_map.png", "non_hair_leakage_map"),
                        ):
                            value = blending_aux[key]
                            if value.size(1) == 2:
                                value = torch.cat((value, torch.zeros_like(value[:, :1])), dim=1)
                            if value.size(1) == 1:
                                value = value.repeat(1, 3, 1, 1)
                            save_gen_image(output_dir, "Blending_v8", name, value / 50.0 - 1.0 if key in ("target_l_final", "anchor_high_frequency_l") else value * 2.0 - 1.0)
                    elif self.v236:
                        save_gen_image(output_dir, "Blending_v8", "hair_chroma_map.png", blending_aux["hair_chroma_map"] * 2.0 - 1.0)
                        save_gen_image(output_dir, "Blending_v8", "hair_illumination_map.png", blending_aux["hair_illumination_map"] * 2.0 - 1.0)
                        save_gen_image(output_dir, "Blending_v8", "illumination_residual.png", blending_aux["illumination_residual"] / self.v236_transfer.max_illumination_shift)
                        save_gen_image(output_dir, "Blending_v8", "chroma_confidence.png", blending_aux["chroma_confidence"] * 2.0 - 1.0)
                        save_gen_image(output_dir, "Blending_v8", "final_leakage_map.png", blending_aux["final_leakage_map"] * 2.0 - 1.0)
                    elif self.v235:
