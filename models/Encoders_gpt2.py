    ...
    def __init__(self, blocks, inplanes=1024):
        ...
        for n, block in enumerate(blocks, start=1):
            ...
            for k in range(1, num_blocks + 1):
                ...
                self.res_blocks[f'res_block_{n}_{k}'] = IBasicBlock(
                    inplanes, planes, 1, downsample, 1, 64, 1
                )
                inplanes = planes
...
class RotateModel(nn.Module):
    def __init__(self):
        ...
        self.modulation_module_list = nn.ModuleList(
            [ModulationModule(6, i == 4) for i in range(5)]
        )
...
class ClipBlendingModel(nn.Module):
    """
    Improved blending encoder:
    replace fixed 0.1 residual with learnable gate.
    """
    ...
    def __init__(self, clip_model="ViT-B/32"):
        ...
        self.clip_model, _ = clip.load(clip_model, device=self.device_name)
    ...
    def get_image_embed(self, image_tensor):
        if not image_tensor.is_cuda and self.device_name == "cuda":
    ...
    def forward(self, latent_face, latent_color, target_face, hair_color):
        ...
        gate_in = torch.cat(
            [
                latent_color.mean(dim=1),
                embed_face.mean(dim=1),
                embed_color.mean(dim=1),
            ],
            dim=-1
        )

        gate = self.out_gate(gate_in).view(-1, 12, 512)
        output = latent_face + gate * dt_latent
        return output
...
class PostProcessModel(nn.Module):
    def __init__(self):
        ...
        self.latent_avg = torch.load(
            'pretrained_models/PostProcess/latent_avg.pt',
...
class ClipModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.device_name = "cuda" if torch.cuda.is_available() else "cpu"
        self.clip_model, _ = clip.load("ViT-B/32", device=self.device_name)
        self.transform = T.Compose(
            [T.Normalize(
                (0.48145466, 0.4578275, 0.40821073),
                (0.26862954, 0.26130258, 0.27577711)
