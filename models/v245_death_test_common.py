...
def write_manifest(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    path.write_text(json.dumps({"version": "v2.45", "count": len(rows), "samples": rows}, indent=2, ensure_ascii=True), encoding="utf-8")


def read_manifest(path: Path) -> list[dict[str, object]]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("version") != "v2.45":
        raise RuntimeError(f"Unsupported V2.45 manifest version: {payload.get('version')!r}")
    return list(payload.get("samples", []))


__all__ = ["composite", "dilate", "erode", "load_cache", "read_manifest", "sample_token", "save_cache", "smoothstep", "write_manifest"]
