...
class BlendingStageV8:
    def __init__(self, model_args):
        ...
        self.blending_encoder = DirectColorBlendAdapterV8(
            ...
            direct_mix_floor=adapter_config.get("direct_mix_floor", model_args.direct_mix_floor_v8),
            correction_budget_ratio=adapter_config.get(
                "correction_budget_ratio", model_args.correction_budget_ratio_v8
            ),
        )
        ...
        print(
            f"[blending_fid_v8] loaded arch={DIRECT_COLOR_ARCH_V8_2} "
    ...
    def blend_images(self, align_shape, align_color, name_to_embed) -> torch.Tensor:
        ...
        _, hair_color_erode = self.dilate_erosion.mask(hair_color)

        latent_s_face = name_to_embed["face"]["S"]
        latent_s_color = name_to_embed["color"]["S"]
        latent_f_align = align_shape["latent_F_align"]
        # Keep evaluation on the same shape/SATD target hair mask used by the
        # v8 trainer and the production Blending_v8 stage.
        aligned_color_hair = align_shape["HM_X"]

        _, aligned_color_hair_erode = self.dilate_erosion.mask(aligned_color_hair)
        ...
        bundle = build_color_condition_bundle(
            reference_image=i_color,
            reference_hair_mask=hair_color_erode,
            base_image=self.downsample_256(i_base),
            target_hair_mask=aligned_color_hair_erode,
...
def build_model() -> BlendingStageV8:
    ...
    model_args.direct_mix_floor_v8 = USER_DIRECT_MIX_FLOOR_V8
    model_args.correction_budget_ratio_v8 = USER_CORRECTION_BUDGET_RATIO_V8
