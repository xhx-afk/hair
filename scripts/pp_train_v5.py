...
USER_DATASET_DIR_SMALL = Path("/root/shared-nvme/hairfast_ppmodify/images/pp_dataset_v5_dual_ear_short_long")
USER_OUTPUT_DIR_SMALL = Path("output/pp_v5_checkpoints_small2_ear_short_long")
USER_RUN_NAME_SMALL = "ear_refine_v5_dual_small_mix"

USER_DATASET_DIR_FULL = Path("images/pp_dataset_v5_dual_full2")
USER_OUTPUT_DIR_FULL = Path("output/pp_v5_checkpoints_full2")
...
USER_EARRING_TARGET_HAIR_OVERRIDE_DILATE = 2
USER_TRUSTED_EARRING_KEEP_DILATE = 2
...
USER_BASE_SOURCE_HAIR_EXCLUDE_DILATE = 9
...
USER_ENABLE_OUTPUT_TARGET_EAR_SKIN_PRESERVE = False
USER_OUTPUT_TARGET_EAR_SKIN_PRESERVE_DILATE = 0
USER_OUTPUT_REVEALED_SKIN_PRESERVE_DILATE = 3
USER_OUTPUT_EARRING_KEEP_DILATE = 2
...
USER_LAMBDA_REVEALED_SKIN_LOW_ANCHOR = 0.0
USER_LAMBDA_REVEALED_SKIN_TEXTURE = 0.0
USER_LAMBDA_REVEALED_SKIN_HIGH = 0.0
USER_LAMBDA_REVEALED_SKIN_ENERGY = 0.0
USER_LAMBDA_REVEALED_SKIN_SEAM = 0.0
...
RESOLVED_USER_CONFIG = {
    ...
    "earring_target_hair_override_dilate": USER_EARRING_TARGET_HAIR_OVERRIDE_DILATE,
    "trusted_earring_keep_dilate": USER_TRUSTED_EARRING_KEEP_DILATE,
    ...
    "enable_output_target_ear_skin_preserve": USER_ENABLE_OUTPUT_TARGET_EAR_SKIN_PRESERVE,
    "output_target_ear_skin_preserve_dilate": USER_OUTPUT_TARGET_EAR_SKIN_PRESERVE_DILATE,
...
def build_parser(defaults):
    ...
    parser.add_argument("--earring_target_hair_override_dilate", type=int, default=defaults["earring_target_hair_override_dilate"])
    parser.add_argument("--trusted_earring_keep_dilate", type=int, default=defaults["trusted_earring_keep_dilate"])
    ...
    parser.add_argument("--enable_output_target_ear_skin_preserve", type=str2bool, default=defaults["enable_output_target_ear_skin_preserve"])
    parser.add_argument("--output_target_ear_skin_preserve_dilate", type=int, default=defaults["output_target_ear_skin_preserve_dilate"])
...
class TrainerV5:
    ...
    def build_preview_row(self, source, target, gen_w_256, gen_f_256, aux, batch, idx):
        ...
        return [
            ...
            self.mask_to_rgb(aux.get("source_earring_object_mask")[idx] if aux.get("source_earring_object_mask") is not None else None, image_size),
            self.mask_to_rgb(aux.get("output_source_earring_composite_mask")[idx] if aux.get("output_source_earring_composite_mask") is not None else None, image_size),
        ]
...
class PPDatasetV5(Dataset):
    ...
    def transform(self, sample):
        ...
        keys_to_flip = ["source", "shape_reference", "color_reference", "target", "earring_reference", "target_mask", "HT_E", "source_hair_mask", "target_hair_mask",
                        ...
                        "visible_ear_roi", "target_covered_ear_block_mask",
...
def item_visible_earring_area(item) -> float:
    visible = item.get("visible_ear_roi", item.get("ear_roi"))
