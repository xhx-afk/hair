...
from models.SelfSimilarity import SelfSimilarityBlendingLoss
...
# ================== 用户配置区域 ==================
config = {
    'name_run': 'blending_sim_train_15fwq',                         # wandb 运行名称
    'dataset': './images/blending',         # 数据集路径（包含 FS, Align 子文件夹）
    'FFHQ': './images/FFHQ',              # FFHQ 图像路径，需替换为实际路径
    'fid_dataset': './fid_images',                       # FID 计算所需的数据集路径
    'epochs': 201,                                # 训练总轮数
    'checkpoint_path': './blending/sim_checkpoint/checkpoint_blending.pth', # 断点保存路径
    'resume': False,                               # 是否从断点恢复
    'save_model_interval': 40,                     # 每多少轮保存一次 blending_n.pth
    'save_val_interval': 20,                         # 每多少轮保存一次验证图片
    'models_dir': 'blending_sim_models',                # 模型权重保存目录
    'val_images_dir': 'blending_sim_files',             # 验证图片保存根目录
    'use_self_similarity': True,                    # 是否使用自相似性损失
    'self_sim_lambda': 0.2,                         # 自相似性损失权重
    'color_lambda': 1.0,                            # 颜色损失权重
    'boundary_lambda': 0.1,                         # 边界损失权重
}
# =================================================
...
class Trainer:
    def __init__(self,
        ...
        # 初始化自相似性损失
        self.use_self_similarity = True  # 默认使用自相似性损失
        self.self_similarity_loss_fn = SelfSimilarityBlendingLoss(
            lambda_self_sim=0.2,
            lambda_color=1.0,
            lambda_boundary=0.1
        )
    ...
    def calc_loss(self, I_gen, I_face, I_color, mask_face, mask_hair, gen_hair):
        if self.use_self_similarity:
            # 使用自相似性损失
            total_loss, loss_dict = self.self_similarity_loss_fn(
                I_gen, I_color, mask_hair, mask_face, self.cur_iter
            )
            ...
            # 合并损失字典
            loss_dict['face loss'] = face_loss.item()
            loss_dict['hair loss'] = hair_loss.item()
        ...
        for key, val in losses.items():
            if key != 'FID CLIP':
                ...
                if isinstance(val, torch.Tensor):
                    ...
                    val = val / len(self.test_dataloader)
...
def main(cfg):
    ...
    # 设置自相似性参数
    trainer.use_self_similarity = cfg.get('use_self_similarity', True)
    if trainer.use_self_similarity:
        trainer.self_similarity_loss_fn = SelfSimilarityBlendingLoss(
            lambda_self_sim=cfg.get('self_sim_lambda', 0.2),
            lambda_color=cfg.get('color_lambda', 1.0),
            lambda_boundary=cfg.get('boundary_lambda', 0.1)
        )
